...
\usepackage{booktabs}
\usepackage[authoryear,round]{natbib} % authorâ€“year citations
\usepackage{qrcode}
\usepackage{listings}
\lstset{
...
\usepackage{pgfpages}
\newif\ifpresenter
\presentertrue    % <â€” uncomment for PRESENTER PDF (two-panel pages)
% \presenterfalse     % <â€” audience PDF (slides only)
...
\title[RMSTpowerBoost]{RMSTpowerBoost: A Comprehensive Software Suite for Power and Sample Size Calculation in Modern Clinical Trials}
\author{Arnab Aich}
...
\begin{frame}
  \titlepage
  \note{
  \textbf{Opening (10s).} Hi everyone, thanks for being here. Iâ€™m Arnab Aich from Florida State University. Today Iâ€™ll walk you through \textbf{RMSTpowerBoost}, a practical toolkit for planning survival trials when proportional hazards may not hold. \\
  \textbf{Agenda (10s).} Weâ€™ll start with motivation, then the RMST idea and causal interpretation, followed by models, power/sample size methods, two examples, and a quick tour of the web app. \\
  \textbf{Transition (2s).} Letâ€™s begin with the \emph{why}.
  }
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{Efficiency:} Trials are costlyâ€”careful design saves time and resources.
  \item \textbf{Science:} Adequate power is essential for trustworthy conclusions.
\end{itemize}
\end{block}

\note{
\textbf{Set-up (20s).} Trial planning always starts with two decisions: sample size and power. These are not just technicalitiesâ€”underpowered studies can waste participant effort and money, and overpowered studies can be unnecessarily large. \\
\textbf{Pause (2s).} Emphasize ethical and scientific responsibility. \\
\textbf{Transition (2s).} With that context, letâ€™s review the standard survival setup.
}
\end{frame}
...
\textbf{Pause (1s).} This is standardâ€”but it sets us up for a common problem with the next slide. \\
\textbf{Transition (1s).} Letâ€™s talk hazards and proportionality.
}
\end{frame}
...
\note{
\textbf{Key point (25s).} PH says hazards differ by a constant factor. Itâ€™s elegant, but in modern trials we often see delayed treatment effects, diminishing effects, or crossing curvesâ€”so the hazard ratio becomes a time-averaged number with murky interpretation. \\
\textbf{Pause (2s).} We need something interpretable when PH fails. \\
\textbf{Transition (1s).} Thatâ€™s where RMST helps.
}
\end{frame}
...
\note{
\textbf{Essence (20s).} RMST tells us â€œhow much event-free time, on average, up to time $L$.â€ Itâ€™s robust to non-proportional hazards and communicates in time unitsâ€”intuitive for clinicians and patients. \\
...
\note{
\textbf{Takeaway (20s).} Our treatment effect is the RMST difference: the average extra event-free time up to $L$. Visually, itâ€™s the difference in the \emph{areas} under the survival curves. This remains valid whether PH holds or not. \\
\textbf{Pause (2s).} With the estimand set, letâ€™s talk models and software.
}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{RMSTpowerBoost R package}: open-source, reproducible implementation.
  \item \textbf{RMSTpowerBoost Shiny web app}: user-friendly interface, no coding required.
\end{itemize}
\end{block}
...
\end{block}
\note{
\textbf{GAM route (20s).} Pseudo-observations yield â€œuncensored-likeâ€ outcomes, so we can use a GAM for flexibility. We retain $\beta_A$ as the RMST difference while letting covariate effects be nonlinear. \\
...
\note{
\textbf{Choice (15s).} Additive model answers â€œhow many months gained?â€; multiplicative answers â€œwhat ratio of RMST?â€. Pick what clinicians find most natural. \\
\textbf{Transition (2s).} Censoring may also depend on covariatesâ€”handle that next.
}
\end{frame}
...
\note{
\textbf{Contrast (20s).} Analytical is fast and great for scanning sample sizes; bootstrap is flexible and validates power under complex settings. In RMSTpowerBoost we support both, often using analytical to search and bootstrap to confirm. \\
\textbf{Transition (2s).} Letâ€™s see two examples.
}
\end{frame}
...
\note{
\textbf{Narrative (25s).} With stratification, we bootstrap power for the RMST ratio over a horizon of five years ($L=1825$). The mean ratio is about 1.0065, CI roughly 0.968â€“1.057 in this pilot. This demonstrates the workflow; real planning would refine assumptions and sample sizes. \\
...
% ---------- Frame 17 ----------
\begin{frame}
\frametitle{RMSTpowerBoost: Web Application Feature}
...
\begin{itemize}
\centering
    \item \textbf{Fully web-based} â€” no R installation required
    \item \textbf{Interactive} plots and \textbf{customizable} visual themes
\end{itemize}

\note{
\textbf{Summary (15s).} The workflow is simple: Input â†’ Configure â†’ Export. You can visualize survival/power curves and customize styling. Itâ€™s fully web-basedâ€”no local R setup required. \\
...
\note{
\textbf{Wrap (20s).} RMSTpowerBoost aims to make robust, interpretable planning routine. We support multiple models and both analytical and bootstrap power/SS calculations, in code and via a GUI. \\
...
\qrcode[height=3cm]{https://uthsc-zhang.github.io/RMSTpowerBoost-Package/}
\end{column}
\end{columns}

\note{
\textbf{Close (20s).} Thanks to our mentors and collaborators, and to the funding support acknowledged here. The QR codes link to the live web app and package documentation. Iâ€™m happy to take questions. \\
...
\note{
Use authorâ€“year citations. If youâ€™d rather hide this slide for time, you can comment it out; the in-slide citations will still render names/years.
}
\end{frame}
